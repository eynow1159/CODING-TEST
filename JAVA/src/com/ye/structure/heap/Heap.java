...
public class Heap<E extends Comparable> implements PQInterface<E> {
	...
	private void percolateUp(int i) {
		// 힙 A[i]에서 시작해서 힙 성질을 만족하도록 수선한다
		// 힙 A[0...i-1]은 힙 성질을 만족하고 있음
		int parent = (i - 1) / 2;
	...
	private void percolateDown(int i) {
		// 힙 A[0...numItems-1]에서 A[i]를 루트로 스며내리기
		int chilld = 2 * i + 1;	// 왼쪽 자식
		int rightChilld = 2 * i + 2;	// 오른쪽 자식
		
		if (chilld <= numItems-1) {
			if (rightChilld <= numItems-1 && A[chilld].compareTo(A[rightChilld]) < 0) {
				chilld = rightChilld;	// 더 큰 자식의 인덱스
			}
			if (A[i].compareTo(A[chilld]) < 0) {
				E tmp =  A[i];
				A[i] = A[chilld];
				A[chilld] = tmp;
				percolateUp(chilld);
